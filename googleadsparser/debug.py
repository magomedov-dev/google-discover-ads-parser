"""Утилиты дампа UI: структура узла в JSON и интерактивный снимок экрана в HTML.

* :func:`save_node` — используется при сборе рекламы (``structure.json`` карточки);
* :func:`dump` — отладочный снимок экрана (CLI ``--dump``) для подбора селекторов.
"""
